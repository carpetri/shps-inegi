...
declare -a files=("ageb_urb" "eje_vial" "estatal" "loc_rur" "loc_urb" "manzanas" "municipal" "servicios_a" "servicios_l" "servicios_p")
...
function reproject {
  for i in "${files[@]}"
  do
    ./ogr2ogr.py $1/$2_$i.shp "$1"/$i.shp -t_srs "$PROJECTION"
    rm "$1"/$i*
  done
